...
```
   ...
   4. לאחר שבחרתם מודל, בדקו את ממוצע ריבועי השגיאות על ה-test set.
      a. כדי לבדוק את סכום ריבוע השגיאות, השתמשו בפונקציה `predict` באופן הבא:
```
predict(linear_model, newdata,...)

```
      b. כדי לחשב את ממוצע ריבועי השגיאות השתמשו ב-mutate כדי לחשב את הוקטור הבא  (price-predicted_price)^2, ואז חשבו ממוצע ערכי הוקטור שקיבלתם.
   5. הקבוצה המנצחת היא זו שהגיעה לשגיאה הנמוכה ביותר.
...
(mean((diamonds_predictions - diamonds$price[-train_set_idx])^2))
...
בפרקי המשך נחזור לרגרסיה הלינארית ונדון בבחירת משתנים ובצמצום מימדים (Variable selection, dimension reduction). כעת בקצרה נמחיש פקודה העוזרת לצמצם את מודל הרגרסיה למשתנים המשמעותיים ביותר:
