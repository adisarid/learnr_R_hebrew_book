...
### תרגיל
   ...
   5. נתבקשתם לבנות מודל שיציע הנחה ללקוחות מסוימים, כדי לצמצם את הנטישה. לאילו לקוחות תציעו הנחה ובאיזה סכום? השתמשו במשתנה monthlycharges כדי לקבוע את שיעור ההנחה (אחוז מתוך משתנה זה, ללקוחות הרלוונטיים).
...
telco_discount %>% 
  filter(Churn == "No") %>%
  summarize(sum(MonthlyCharges))

discount_rate <- 0.85
discount_quantile <- 0.75
pred_quantile <- as.numeric(quantile(telco_discount$predicted_churn, discount_quantile))

telco_discount <- telco_discount %>% 
  mutate(top25churn = predicted_churn >= pred_quantile) %>%
  mutate(originalMonthlyCharges = MonthlyCharges) %>%
  mutate(MonthlyCharges = 
           ifelse(top25churn, originalMonthlyCharges*discount_rate, originalMonthlyCharges))

telco_discount <- telco_discount %>%
  mutate(predicted_churn_discount = 
           predict(churn_detection, newdata = ., type = "response"))

sum(telco_discount$predicted_churn_discount*telco_discount$MonthlyCharges)
sum(telco_discount$predicted_churn*telco_discount$originalMonthlyCharges)
  

# The measure we are going to use is expected value. We are going to use the logistic regression
# which is the best classifier here. 
# first, we will omit the total charges (because they are related to tenure and monthly price), 
# and build a new model (using a train/test split).
# Using the test set we're going to modify the monthly charges of all users with the highest 25% chance of churning by offering a 15% discount, and see if it is profitable to offer these discounts.
